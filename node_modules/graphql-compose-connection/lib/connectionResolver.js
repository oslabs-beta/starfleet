...
function prepareConnectionResolver(tc, opts) {
  ...
  return tc.schemaComposer.createResolver({
    type: (0, _connectionType.prepareConnectionType)(tc, opts.connectionResolverName),
    ...
    async resolve(resolveParams) {
      ...
      if (projection && projection.edges) {
        ...
        findManyParams.projection = _objectSpread({}, projection, projection.edges.node);
      } else {
      ...
      return Promise.all([findManyPromise, countPromise]).then(([recordList, count]) => {
        const edges = []; // transform record to object { cursor, node }
