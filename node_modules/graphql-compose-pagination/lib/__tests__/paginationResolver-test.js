...
describe('paginationResolver', () => {
  const spyFindManyResolve = jest.spyOn(_User.UserTC.getResolver('findMany'), 'resolve');
  const spyCountResolve = jest.spyOn(_User.UserTC.getResolver('count'), 'resolve');
  const paginationResolver = (0, _paginationResolver.preparePaginationResolver)(_User.UserTC, {
  ...
  describe('definition checks', () => {
    it('should return Resolver', () => {
      expect(paginationResolver).toBeInstanceOf(_graphqlCompose.Resolver);
    });
    it('should throw error if first arg is not ObjectTypeComposer', () => {
      expect(() => {
        const args = [123];
        (0, _paginationResolver.preparePaginationResolver)(...args);
    ...
    it('should throw error if resolver opts.countResolverName does not exists', () => {
      expect(() => (0, _paginationResolver.preparePaginationResolver)(_User.UserTC, {
  ...
  describe('resolver basic properties', () => {
    ...
    it('should have type to be ConnectionType', () => {
      expect(paginationResolver.type.getTypeName()).toBe('UserPagination');
    });
  });
  describe('resolver args', () => {
    it('should have `page` arg', () => {
      expect(paginationResolver.getArgType('page')).toBe(_graphql.GraphQLInt);
    });
    it('should have `perPage` arg', () => {
      expect(paginationResolver.getArgType('perPage')).toBe(_graphql.GraphQLInt);
    });
  });
  describe('call of resolvers', () => {
    ...
    beforeEach(() => {
      ...
      const mockedFindMany = _User.UserTC.getResolver('findMany').wrapResolve(next => resolveParams => {
        findManyResolverCalled = true;
        spyResolveParams = resolveParams;
        return next(resolveParams);
      });

      const mockedCount = _User.UserTC.getResolver('findMany').wrapResolve(next => resolveParams => {
        countResolverCalled = true;
        spyResolveParams = resolveParams;
        return next(resolveParams);
      });

      _User.UserTC.setResolver('mockedFindMany', mockedFindMany);

      _User.UserTC.setResolver('mockedCount', mockedCount);

      mockedpaginationResolver = (0, _paginationResolver.preparePaginationResolver)(_User.UserTC, {
